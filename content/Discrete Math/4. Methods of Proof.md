...
- We select an arbitrarily element from the domain (D) then prove that $P(x)$ holds for the that arbitrarily element. Since the element was selected arbitrarily it means $P(x)$ holds for all elements in domain (D). See [[Example Proofs]] for some examples on using Universal Generalization
