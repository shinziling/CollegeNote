...
This is the same idea with ordinal (which is based on natural number) except it is commonly used to deal with numbers that represent "infinity".
- This is generalization of ordinal numeral (first, second, third, etc where each corresponds to a natural number, zeroth would corresponds to 0) but extending to infinite set.
